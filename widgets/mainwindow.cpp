...
namespace
{
  AudioDevice::Channel effective_audio_output_channel (AudioDevice::Channel channel)
  {
#if QT_VERSION >= QT_VERSION_CHECK (6, 0, 0) && defined (Q_OS_WIN)
    // Qt6's Windows audio backend commonly exposes stereo-only output
    // formats. Preserve audio by duplicating a saved mono stream to both
    // output channels.
  ...
  m_useDarkStyle {false}
{
  ...
  if (!m_tci_audio) {
    ...
    if (!m_config.audio_output_device ().isNull ())
      {
        auto const output_channel = effective_audio_output_channel (m_config.audio_output_channel ());
        Q_EMIT initializeAudioOutputStream (m_config.audio_output_device ()
                                            , AudioDevice::Mono == output_channel ? 1 : 2
...
{
  static QFile f {QDir {QStandardPaths::writableLocation (QStandardPaths::AppDataLocation)}.absoluteFilePath ("wsjtx.log")};
...
void MainWindow::rigFailure (QString const& reason)
{
  if (m_first_error)
    {
      // one automatic retry
      QTimer::singleShot (1500, this, SLOT (rigOpen ()));
      m_first_error = false;
    }
  else
    {
  ...
  if (m_mode == "JT65") {
    ...
    if (m_tci_audio) {
      ...
      Q_EMIT sendMessage (m_mode, NUM_JT65_SYMBOLS,
             4096.0*12000.0/11025.0, ui->TxFreqSpinBox->value () - m_XIT,
             toneSpacing, m_soundOutput, effective_audio_output_channel (m_config.audio_output_channel ()),
  ...
  if (m_mode == "FST4" or m_mode == "FST4W") {
    ...
    if (m_tci_audio) {
      ...
      Q_EMIT sendMessage (m_mode, NUM_FST4_SYMBOLS,double(nsps),f0,toneSpacing,
                          m_soundOutput, effective_audio_output_channel (m_config.audio_output_channel ()),
                          true, false, snr, m_TRperiod);
    }
  }
  ...
  if (m_mode=="WSPR") {
    ...
    if (m_tci_audio) {
      ...
      Q_EMIT sendMessage (m_mode, NUM_WSPR_SYMBOLS, 8192.0,
                          ...
                          effective_audio_output_channel (m_config.audio_output_channel ()), true, false, snr,
                          m_TRperiod);
    }
  }
  ...
  if(m_mode=="Echo") {
    ...
    if (m_tci_audio) {
      ...
      Q_EMIT sendMessage (m_mode,numEchoSymbols,framesPerSymbol,freq,toneSpacing,m_soundOutput,
                          effective_audio_output_channel (m_config.audio_output_channel ()), false, false, snr, m_TRperiod);
    }
  }
...
QChar MainWindow::current_submode () const
{
  ...
  if (m_mode.contains (QRegularExpression {R"(^(JT65|JT9|JT4|Q65)$)"})
    ...
    {
      submode = QChar {static_cast<char16_t> (m_nSubMode + 65)};
    }
  return submode;
}
...
void MainWindow::p1ReadFromStdout()                        //p1readFromStdout
{
  ...
  while(p1.canReadLine()) {
    ...
    if(t.indexOf("<DecodeFinished>") >= 0) {
      ...
      QStringList rxFields = t.split(QRegularExpression {"\\s+"}, Qt::SkipEmptyParts);
      QString rxLine;
      QString grid="";
      if ( rxFields.count() == 8 ) {
...
{
  QString t;
  t = t.asprintf("%3d%3d%3d",static_cast<int> (m_houndQueue.count()),static_cast<int> (m_foxQSOinProgress.count()),static_cast<int> (m_foxQSO.count()));
...
void MainWindow::foxTest()
{
  ...
  while(!s.atEnd()) {
    ...
    if(line.contains("Tx1:")) {
      ...
      t = t.asprintf("%3d %3d %3d %3d %5d   ",static_cast<int> (m_houndQueue.count()),
                static_cast<int> (m_foxQSOinProgress.count()),static_cast<int> (m_foxQSO.count()),
...
{
#if defined(Q_OS_WIN)
    QFile::remove (QDir {QStandardPaths::writableLocation (QStandardPaths::AppDataLocation)}.absoluteFilePath ("wsjtx_log_config.ini"));
#else
...
{
#if defined(Q_OS_WIN)
    static QFile f {QDir {QStandardPaths::writableLocation (QStandardPaths::AppDataLocation)}.absoluteFilePath ("wsjtx_log_config.ini")};
#else
...
void MainWindow::on_actionDisable_event_logging_triggered()
{
    ...
    out << EventConfig;
    f.close();
    QFile::remove (QDir {QStandardPaths::writableLocation (QStandardPaths::AppDataLocation)}.absoluteFilePath ("wsjtx_syslog.log"));
}
...
{
    static QFile logfile {QDir {QStandardPaths::writableLocation (QStandardPaths::AppDataLocation)}.absoluteFilePath ("wsjtx.log")};
...
{
  if (m_hisCall!="") {
    static QFile ignoreFile {QDir {QStandardPaths::writableLocation (QStandardPaths::AppDataLocation)}.absoluteFilePath ("ignore.list")};
...
void MainWindow::remove_old_files(const QString &directoryPath, int daysOld)
{
    ...
    for (QFileInfo const& fileInfo : fileList) {

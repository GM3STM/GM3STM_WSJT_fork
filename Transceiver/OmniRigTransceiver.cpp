...
namespace
{
  ...
  bool wait_for_omnirig (std::function<bool ()> const& ready, int timeout_ms)
  {
    QElapsedTimer timer;
    timer.start ();
  ...
  int ax_int (QAxObject * object, char const * property)
  {
    return object->property (property).toInt ();
  }

  bool ax_bool (QAxObject * object, char const * property)
  {
    return object->property (property).toBool ();
  }

  QString ax_string (QAxObject * object, char const * property)
  {
    return object->property (property).toString ();
  }

  qint64 ax_frequency (QAxObject * object, char const * property)
  {
    return object->property (property).toLongLong ();
  }

  qint64 ax_frequency_call (QAxObject * object, char const * method)
  {
  ...
  try
    {
      ...
      if (SUCCEEDED (hr))
        ...
        {
          CAT_ERROR ("failed to initialize COM apartment for OmniRig: " << hr);
          throw_qstring (tr ("Failed to initialize COM apartment for OmniRig"));
        }

      omni_rig_.reset (new QAxObject {"OmniRig.OmniRigX", this});
      if (omni_rig_->isNull ())
        {
      ...
      auto const software_version = ax_int (omni_rig_.data (), "SoftwareVersion");
      auto const interface_version = ax_int (omni_rig_.data (), "InterfaceVersion");
      CAT_INFO ("OmniRig s/w version: " << static_cast<quint16> (software_version >> 16)
                << '.' << static_cast<quint16> (software_version & 0xffff)
                << " i/f version: " << static_cast<int> (interface_version >> 8 & 0xff)
                << '.' << static_cast<int> (interface_version && 0xff));

      // fetch the interface of the RigX CoClass and instantiate a proxy object
      switch (rig_number_)
        {
        case One: rig_.reset (omni_rig_->querySubObject ("Rig1")); break;
        case Two: rig_.reset (omni_rig_->querySubObject ("Rig2")); break;
        }

      if (!rig_ || rig_->isNull ())
        {
          CAT_ERROR ("failed to access OmniRig rig interface");
          throw_qstring (tr ("Failed to access OmniRig rig interface"));
        }

      // Qt 6 ActiveQt can crash while advising the generic exception
      // signal on OmniRig subobjects. The top-level OmniRig exception
      // connection above remains active.
      ...
      if (use_for_ptt_ && (TransceiverFactory::PTT_method_DTR == ptt_type_ || TransceiverFactory::PTT_method_RTS == ptt_type_))
        {
          ...
          port_.reset (rig_->querySubObject ("PortBits"));

          if (!port_ || port_->isNull ())
            {
              CAT_ERROR ("failed to access OmniRig serial port interface");
              throw_qstring (tr ("Failed to access OmniRig serial port interface"));
            }

          // See the Rig exception connection note above.

          CAT_TRACE ("OmniRig RTS state: " << ax_bool (port_.data (), "Rts"));
      ...
      rig_type_ = ax_string (rig_.data (), "RigType");
      readable_params_ = ax_int (rig_.data (), "ReadableParams");
      writable_params_ = ax_int (rig_.data (), "WriteableParams");
      ...
      CAT_INFO (QString {"OmniRig initial rig type: %1 readable params=0x%2 writable params=0x%3 for rig %4"}
         ...
         .arg (rig_number_));
      auto const rx_frequency = ax_frequency_call (rig_.data (), "GetRxFrequency()");
      update_rx_frequency (rx_frequency);
      int resolution {0};
      if (OmniRig::PM_UNKNOWN == ax_int (rig_.data (), "Vfo")
  ...
  if (port_)
    {
      ...
      if (!port_->isNull ()) port_->clear ();
      port_.reset ();
    }
  if (rig_)
    {
      if (!rig_->isNull ()) rig_->clear ();
      rig_.reset ();
      CAT_TRACE ("rig_ reset");
    }
  if (omni_rig_)
    {
      if (!omni_rig_->isNull ()) omni_rig_->clear ();
      omni_rig_.reset ();
    }
...
{
  auto const message = tr ("OmniRig COM/OLE error: %1 at %2: %3 (%4)")
    .arg (QString::number (code)).arg (source).arg (desc).arg (help);
  CAT_ERROR (message);

  // ActiveQt emits exceptions from inside its event callback. Defer shutdown
  // until that callback has unwound rather than throwing across Qt internals.
  QTimer::singleShot (0, this, [this, message] {offline (message);});
}

void OmniRigTransceiver::handle_visible_change ()
{
  if (!omni_rig_ || omni_rig_->isNull ()) return;
  CAT_TRACE ("visibility change: visibility =" << omni_rig_->property ("DialogVisible").toBool ());
}
  ...
  if (rig_number_ == rig_number)
    {
      ...
      CAT_INFO (QString {"rig type change to: %1 readable params = 0x%2 writable params = 0x%3 for rig %4"}
        .arg (ax_string (rig_.data (), "RigType"))
        .arg (readable_params_, 8, 16, QChar ('0'))
        .arg (writable_params_, 8, 16, QChar ('0'))
        .arg (rig_number));
    }
...
void OmniRigTransceiver::handle_status_change (int rig_number)
{
  ...
  if (rig_number_ == rig_number)
    {
      if (!rig_) return;
      auto const& status = ax_string (rig_.data (), "StatusStr");
      CAT_TRACE ("OmniRig status change: new status = " << status);
      if (OmniRig::ST_ONLINE != ax_int (rig_.data (), "Status"))
        {
  ...
  if (rig_number_ == rig_number)
    {
      ...
      if (params & OmniRig::PM_VFOAA)
        {
          ...
          update_split (false);
          reversed_ = false;
          update_rx_frequency (ax_frequency (rig_.data (), "FreqA"));
          update_other_frequency (ax_frequency (rig_.data (), "FreqB"));
        }
      ...
      if (params & OmniRig::PM_VFOBA)
        {
          ...
          update_split (true);
          reversed_ = true;
          update_other_frequency (ax_frequency (rig_.data (), "FreqA"));
          update_rx_frequency (ax_frequency (rig_.data (), "FreqB"));
        }
      ...
      if (params & OmniRig::PM_FREQA)
        {
          auto f = ax_frequency (rig_.data (), "FreqA");
          CAT_TRACE ("FREQA = " << f);
          if (reversed_)
            {
      ...
      if (params & OmniRig::PM_FREQB)
        {
          auto f = ax_frequency (rig_.data (), "FreqB");
          CAT_TRACE ("FREQB = " << f);
          if (reversed_)
            {
              update_rx_frequency (f);
      ...
      if (need_frequency)
        {
          ...
          if (readable_params_ & OmniRig::PM_FREQ && !state ().ptt ())
            {
              auto f = ax_frequency (rig_.data (), "Freq");
              if (f)
                {
                  CAT_TRACE ("FREQ = " << f);
                  update_rx_frequency (f);
      ...
      if (params & OmniRig::PM_VFOEQUAL)
        {
          auto f = readable_params_ & OmniRig::PM_FREQA ? ax_frequency (rig_.data (), "FreqA") : ax_frequency (rig_.data (), "Freq");
          auto m = map_mode (static_cast<OmniRig::RigParamX> (ax_int (rig_.data (), "Mode")));
      ...
      if (params & OmniRig::PM_VFOSWAP)
        {
          ...
          update_rx_frequency (f);
          update_mode (map_mode (static_cast<OmniRig::RigParamX> (ax_int (rig_.data (), "Mode"))));
        }
  ...
  else
    {
      if (port_)
      ...
      else if (wrapped_)
        {
          if (!wrapped_started_)
            {
              CAT_TRACE ("starting basic transceiver lazily for PTT");
              wrapped_->start (0);
              wrapped_started_ = true;
            }
  ...
  if (split)
    {
      ...
      rig_->dynamicCall ("SetSplitMode(int,int)", static_cast<int> (state ().frequency ()), static_cast<int> (tx));
  ...
  else
    {
      ...
      rig_->dynamicCall ("SetSimplexMode(int)", static_cast<int> (state ().frequency ()));
      update_split (false);
    }
  bool notify {false};

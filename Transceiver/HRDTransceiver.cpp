...
namespace
{
  ...
  int index_of_match (QStringList const& list, QRegularExpression const& re)
  {
    for (int i = 0; i < list.size (); ++i)
      {
        if (re.match (list.at (i)).hasMatch ())
          {
            return i;
          }
      }
...
{
  return index_of_match (buttons_, re);
}

int HRDTransceiver::find_dropdown (QRegularExpression const& re) const
{
  return index_of_match (dropdown_names_, re);
}
